...
bool isInsufficientSpaceError(const std::string& msg) {
    ...
    // Language-agnostic: English + Spanish (es.lang pure or bilingual) + common variants.
    // InstallController always setStage("Space") for this path; call sites also check stage.
    std::string m = msg;
    ...
    return m.find("not enough free space") != std::string::npos
        ...
        // Spanish (es.lang): "No hay espacio libre en ux0. Se necesitan ~X libres..."
        || m.find("no hay espacio") != std::string::npos
        || m.find("espacio libre en ux0") != std::string::npos
        || m.find("libera unos") != std::string::npos
        // Portuguese (if translated later)
        || m.find("espaco insuficiente") != std::string::npos
        || m.find("espaço livre") != std::string::npos
        || m.find("espaco livre") != std::string::npos;
}
...
bool isNonReportableInstallError(const std::string& msg) {
    ...
    return m.find("no internet connection") != std::string::npos
        // Spanish: "Sin conexión a internet..." (ASCII fold leaves ó; "sin conex" still matches)
        || m.find("sin conex") != std::string::npos
        || m.find("sin internet") != std::string::npos
...
void FullCatalogScreen::trySendErrorReport(const std::string& title, const std::string& context) {
    ...
    // Hard block: user-fixable space / offline errors must never hit Discord.
    // Covers install overlay Report, SQUARE shortcut, and footer Report chip.
    {
        const bool spaceBlocked = (installProgressStage_ == "Space")
            || isInsufficientSpaceError(installProgressMessage_)
            || isInsufficientSpaceError(context)
            || isInsufficientSpaceError(title);
        const bool otherNonReportable = isNonReportableInstallError(installProgressMessage_)
            || isNonReportableInstallError(context)
            || isNonReportableInstallError(title);
        if (spaceBlocked || otherNonReportable) {
            diagnostics::log(std::string("[UI] error report blocked (non-reportable")
                             + (spaceBlocked ? ": space" : ": offline/network")
                             + "): " + title);
            if (spaceBlocked)
                showToast(::psvitaalive::L(::psvitaalive::TextId::FreeSpaceHint1), 2200);
            return;
        }
    }
...
void FullCatalogScreen::completeSearch(const std::string& query){
    applySearch(query);
}
...
void FullCatalogScreen::moveCatalogFocus(int d){if(catalogView().empty())return;if(state_.mode!=UiMode::FULL_CATALOG)releaseScreenshotTextures();if(state_.mode==UiMode::FULL_CATALOG){if(d<0&&state_.focusIndex>=3)state_.focusIndex-=3;if(d>0&&state_.focusIndex+3<(int)catalogView().size())state_.focusIndex+=3;}else{if(d<0&&state_.focusIndex>0)--state_.focusIndex;if(d>0&&state_.focusIndex+1<(int)catalogView().size())++state_.focusIndex;}clampCatalogFocus();clampCatalogScroll();state_.detailScroll=0;detailScrollBeforeLinkMode_=0;state_.linkFocus=-1;state_.linkNavigation=false;}void FullCatalogScreen::moveDetailScroll(int d){state_.detailScroll+=d<0?-72:72;clampDetailScroll();}void FullCatalogScreen::enterLinkNavigation(){
    ...
    const int linkDetailIndex = idxs[li];
    const CatalogLink&l=item.linkDetails[linkDetailIndex];
    ...
    if ((state_.catalog == CatalogType::PspGames || state_.catalog == CatalogType::Ps1Games)
        && !settingsEdit_.pspSetupDone) {
        openPspSetupWizard(false, i, linkDetailIndex);
        return;
    }
    applyPspLiveAreaPluginGateOrRun(false, i, linkDetailIndex);
}

bool FullCatalogScreen::guardPspPs1SetupOrProceed(bool forInstallAll, int itemIndex, int linkDetailIndex) {
    if ((state_.catalog != CatalogType::PspGames && state_.catalog != CatalogType::Ps1Games))
        return true;
    if (settingsEdit_.pspSetupDone) return true;
    openPspSetupWizard(forInstallAll, itemIndex, linkDetailIndex);
    return false;
}

void FullCatalogScreen::openPspSetupWizard(bool forInstallAll, int itemIndex, int linkDetailIndex) {
    pspSetupModal_ = true;
    pspSetupPendingInstallAll_ = forInstallAll;
    pspSetupPendingItemIndex_ = itemIndex;
    pspSetupPendingLinkIndex_ = linkDetailIndex;
    pspSetupTarget_ = settingsEdit_.pspTarget;
    pspSetupMedia_ = settingsEdit_.pspMediaFormat;
    pspSetupFocus_ = (pspSetupTarget_ == ::psvitaalive::PspTarget::LiveArea) ? 0 : 1;
    diagnostics::log("[UI] PSP/PS1 setup wizard opened");
}

void FullCatalogScreen::applyPspLiveAreaPluginGateOrRun(bool forInstallAll, int itemIndex, int linkDetailIndex) {
    if (itemIndex < 0 || itemIndex >= (int)catalogView().size()) return;
    const CatalogItem& item = catalogView()[itemIndex];

    if (forInstallAll) {
        openInstallAllWizard();
        return;
    }
    if (linkDetailIndex < 0 || linkDetailIndex >= (int)item.linkDetails.size()) return;
    const CatalogLink& l = item.linkDetails[linkDetailIndex];
    if (!linkAction_ || !actionableLink(l)) return;
...
void FullCatalogScreen::closePspSetupWizard(bool confirm) {
    ...
    diagnostics::log(std::string("[UI] PSP/PS1 setup saved target=")
                     + ::psvitaalive::AppSettings::toString(settingsEdit_.pspTarget)
                     + " media=" + ::psvitaalive::AppSettings::toString(settingsEdit_.pspMediaFormat));

    const bool forAll = pspSetupPendingInstallAll_;
    const int itemIndex = pspSetupPendingItemIndex_;
    const int linkIndex = pspSetupPendingLinkIndex_;
    pspSetupPendingInstallAll_ = false;
    pspSetupPendingItemIndex_ = -1;
    pspSetupPendingLinkIndex_ = -1;
...
// --- Header search bar + G/D Files filter (Homebrew only) ---
    if (y < HEADER_H) {
        ...
        if (hit(x, y, barX, barY, barW, barH)) {
            ...
            if (searchRequest_) searchRequest_(searchQuery_);
            return;
        }
        return;
    }
...
LocalInstallInfo FullCatalogScreen::queryLocalInstall(const CatalogItem& item) {
    ...
    if (!foundAppDir) {
        ...
        ::psvitaalive::update::UpdateDetectionMeta meta{}; // no catalog signatures yet → fallback policy
        ::psvitaalive::update::ReceiptEvidence receiptEv{};
        ::psvitaalive::update::InstallReceipt diskReceipt{};
        if (::psvitaalive::update::InstalledReleaseTracker::readReceipt(tid, diskReceipt)) {
            receiptEv.present = true;
            receiptEv.catalogVersion = diskReceipt.catalogVersion;
            receiptEv.versionDate = diskReceipt.versionDate;
            receiptEv.releaseRevision = diskReceipt.releaseRevision;
            // Phase 1: version receipts without digest are accepted only to confirm
            // "we installed this catalog version". Older receipts without digest do
            // not alone force UpdateAvailable (decideInstallState + empty digest).
            if (diskReceipt.verifyDigest.empty()) {
                receiptEv.fingerprintMatchesInstalled =
                    (!diskReceipt.catalogVersion.empty() &&
                     !item.version.empty() &&
                     compareVersionStrings(diskReceipt.catalogVersion, item.version) == 0);
            } else {
                // Digest present but live re-hash not yet wired → treat as unverified.
                receiptEv.fingerprintMatchesInstalled = false;
            }
        }
        ::psvitaalive::update::FingerprintEvidence fpEv{};
        ::psvitaalive::update::SfoEvidence sfoEv{};
        sfoEv.hasAppVer = !ver.empty();
        sfoEv.appVer = ver;

        const auto decided = ::psvitaalive::update::decideInstallState(
            true, item.version, meta, receiptEv, fpEv, sfoEv);

        switch (decided.state) {
        case ::psvitaalive::update::InstallDetectState::NotInstalled:
            info.state = LocalInstallState::NotInstalled; break;
        case ::psvitaalive::update::InstallDetectState::Installed:
            info.state = LocalInstallState::Installed; break;
        case ::psvitaalive::update::InstallDetectState::UpdateAvailable:
            info.state = LocalInstallState::UpdateAvailable; break;
        case ::psvitaalive::update::InstallDetectState::InstalledUnknown:
            info.state = LocalInstallState::InstalledUnknown; break;
        default:
            info.state = LocalInstallState::InstalledUnknown; break;
        }
        if (!decided.installedVersion.empty())
            info.installedVersion = decided.installedVersion;
    }
...
void FullCatalogScreen::drawDetailContent(const CatalogItem& it, int x, int y, int w, int h) {
    ...
    std::string installLine;
    {
        ...
        if (li.state == LocalInstallState::Installed || li.state == LocalInstallState::UpdateAvailable || li.state == LocalInstallState::InstalledUnknown) {
            if (li.state == LocalInstallState::UpdateAvailable)
                installLine = ::psvitaalive::L(::psvitaalive::TextId::InstallStateUpdateAvailable);
            else if (li.state == LocalInstallState::InstalledUnknown)
                installLine = ::psvitaalive::L(::psvitaalive::TextId::InstallStateInstalledUnknown);
            else
                installLine = ::psvitaalive::L(::psvitaalive::TextId::InstallStateInstalled);
...
void FullCatalogScreen::drawPspSetupOverlay() {
